...
function computeCoverageForFen(
    ...
    if (isUserTurn) {
        ...
        for (const childFen of childFenMap.values()) {
            const child = computeCoverageForFen(childFen, dbCache, stateMoves, orientation, minGames, memo);
...
function buildPathMaps(
    ...
    dbMovesMap: Map<string, { moves: { move: string; white: number; draw: number; black: number }[]; total: number }>;
} {
    const coverageMap = new Map<string, number>();
    const gamesMap = new Map<string, number>();
    const missingGamesMap = new Map<string, number>();
    const dbMovesMap = new Map<string, { moves: { move: string; white: number; draw: number; black: number }[]; total: number }>();
